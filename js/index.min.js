  ...
  !(function () {
    ...
    function t(t) {
      ...
      const s = document.querySelector(".burger__desk .burger__open--one"),
        l = document.querySelector(".burger__desk .burger__open--two"),
        a = document.querySelector(".burger__desk .burger__open--three");
      n
        ? (gsap.to(s, { duration: 0.3, scaleX: 1, x: 0 }),
          gsap.to(l, { duration: 0.3, scaleX: 1, x: 0 }),
          gsap.to(a, { duration: 0.3, scaleX: 1, x: 0 }))
        : (gsap.to(l, { duration: 0.3, scaleX: 1.3, x: "-5px" }),
          gsap.to(a, { duration: 0.3, scaleX: 0.8, x: "4px" }));
      const c = n ? e : o;
      return gsap.to(c, {
        duration: 0.3,
        stagger: 0.05,
...
document.querySelector(".scroll-perc").addEventListener("click", (o) => {
  const r = o.currentTarget.getBoundingClientRect();
  let t = o.clientY - r.top;
  t = (t / 160) * 100;
  const s = Math.floor((t * n) / 100);
  e.scrollTo(s);
});
let t = 0,
  n = 0;
e.on("scroll", ({ scroll: e, limit: o }) => {
  (n = o.y), (t = (e.y / o.y) * 100 - 100);
  const r = t + "%";
  gsap.set(".fill", { y: r });
}),
